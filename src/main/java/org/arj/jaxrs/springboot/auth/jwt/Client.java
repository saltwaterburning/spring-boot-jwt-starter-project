...
public class Client {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println("JWT token: " + publicKey);

			String response = client.request().target(API_TARGET).path("roles").authorizationBearer(publicKey)
					.getForEntity(String.class).orElse(null);

			System.out.println("Roles: " + response);

			response = client.request().target(API_TARGET).path("role1").authorizationBearer(publicKey)
					.getForEntity(String.class).orElse(null);

			System.out.println("Role1: " + response);

			ResponseEntity<String> re = client.request().target(API_TARGET).path("role2").authorizationBearer(publicKey)
					.get(String.class);
